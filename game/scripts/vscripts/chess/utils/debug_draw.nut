...
::debug_draw <- {
    ...
    function box(p, min=Vector(-0.5,-0.5,-0.5), max=Vector(0.5,0.5,0.5), RGB=[255, 0, 0, 255], draw_time=null) {
        if (!draw_time) draw_time = 0;
        DebugDrawBox(p, min, max, RGB[0], RGB[1], RGB[2], RGB[3], draw_time)
    }
    function box_angles(p, min=Vector(-0.5,-0.5,-0.5), max=Vector(0.5,0.5,0.5), angles=Vector(0, 0, 0), RGB=[255, 0, 0, 255], draw_time=null) {
        if (!draw_time) draw_time = 0;
