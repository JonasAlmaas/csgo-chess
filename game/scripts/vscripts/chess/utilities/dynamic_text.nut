...
::new_dynamic_text <- function (message, char_size, font, color, align, pos, ang) {
    ...
    for (local i = 0; i < message.len(); i++) {
        ...
        else if (Util_list_contains(char, supported_chars["upper"])) {
            offset_distance_step = (char_size * char_size_factor[font]["upper"][char]) / 2
            sub_folder = "/upper/"
        }
        else if (Util_list_contains(char, supported_chars["lower"])) {
            offset_distance_step = (char_size * char_size_factor[font]["lower"][char]) / 2
            sub_folder = "/lower/"
        }
        else if (Util_list_contains(char, supported_chars["number"])) {
            offset_distance_step = (char_size * char_size_factor[font]["number"]) / 2
            sub_folder = "/char/num"
        }
        else if (Util_list_contains(char, supported_chars["char"])) {
            local index = 0
    ...
    foreach (char_info in char_info_list) {
        ...
        if (char == " ") {
            ...
            if (Util_list_contains(char, supported_chars["upper"])) width += char_size * char_size_factor[font]["upper"][char]
            else if (Util_list_contains(char, supported_chars["lower"])) width += char_size * char_size_factor[font]["lower"][char]
            else if (Util_list_contains(char, supported_chars["number"])) width += char_size * char_size_factor[font]["number"]
            else if (Util_list_contains(char, supported_chars["char_name"])) width += char_size * char_size_factor[font]["char"][char]
        }
    }
    ...
    local text_table = {
        ...
        function create(pos, ang) {
            local offset_distance = Util_vec_clone(alignment_offset)

            foreach (char_info in char_info_list) {
                if (char_info["char"] != " ") {
                    local index = 0
                    local available_prop = false

                    while (!available_prop && index < custom_entities.prop_dynamic.len()) {
                        if (custom_entities.prop_dynamic[index].is_disabled) {
                            available_prop = true
                        } else {
                            index++
                        }
                    ...
                    local item = null
                    if (available_prop) {
                        item = custom_entities.prop_dynamic[index]
                        item.is_disabled = false
                        EntFireByHandle(item.handle, "Enable", "", 0.0, null, null)
                    } else {
                        local handle = Entities.CreateByClassname("prop_dynamic")
                        handle.__KeyValueFromInt("disableshadowdepth", 1)
                        item = { handle = handle, is_disabled = false }
                        custom_entities.prop_dynamic.append(item)
                    }

                    offset_distance.x += char_info["offset_distance_step"]
                    local offset = Util_vec_rotate_3d(Vector(0, offset_distance.x, offset_distance.y), ang)

                    item.handle.SetOrigin(pos + offset)
                    item.handle.SetAngles(ang.x, ang.y, ang.z)
                    item.handle.SetModel("models/text/" + font + char_info["sub_folder"] + char_info["char"] + ".mdl")
                    item.handle.__KeyValueFromFloat("modelscale", char_size)
                    EntFireByHandle(item.handle, "Color", Util_get_string_from_color(color), 0.0, null, null)
        ...
        function kill() {
            foreach (item in item_list) {
                ...
                EntFireByHandle(item.handle, "Disable", "", 0.0, null, null)
            }
            item_list = []
        }
